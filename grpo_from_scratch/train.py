...
class GSM8KDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # prompt = sample["question"]     # 英文问题
        # answer = sample["answer"]       # 含推理过程的回答
        answer = sample["answer_only"]  # 只有答案的回答
...
class GRPOTrainer:
    ...
    def generate_experiences(self, inputs):
        ...
        return {
            ...
            "ref_action_log_probs": (
                torch.cat(batch_ref_action_log_probs, dim=0) if self.ref_model else None
            ),
    ...
    def train_step(self, model, inputs, optimizer, step):
        ...
        if (step + 1) % self.args.gradient_accumulation_steps == 0:
            ...
            print(
                f"step: {self.update_steps}/{self.global_steps}  grpo_loss: {loss.item():.8f}"
            )
